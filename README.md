...
- 💳 Система подписок с поддержкой двух способов оплаты:
  - ⭐ Telegram Stars (оплата внутри Telegram)
  - 💳 Банковские карты через YooMoney (Visa, MasterCard, Mir)
- 👥 Whitelist для бесплатного доступа избранных пользователей
...
- `/check_payment` - Проверить статус оплаты картой (если оплата ещё не завершена)
...
- `yookassa` - Для интеграции с платежной системой YooMoney

## 💳 Настройка платежей

Бот поддерживает два способа оплаты подписки:

### 1. Telegram Stars (⭐)
Работает "из коробки", не требует дополнительной настройки.

### 2. Банковские карты через YooMoney (💳)

Для настройки оплаты картами:

1. Зарегистрируйтесь на [YooMoney (YooKassa)](https://yookassa.ru/)
2. Создайте магазин и получите credentials
3. Добавьте в `.env` файл:
   ```env
   YOOKASSA_SHOP_ID=ваш_shop_id
   YOOKASSA_SECRET_KEY=ваш_secret_key
   YOOKASSA_TEST_MODE=True  # False для боевого режима
   SUBSCRIPTION_PRICE_RUB=150.00
   ```
...
**Подробная документация:** см. [YOOMONEY_INTEGRATION.md](YOOMONEY_INTEGRATION.md)

### Webhook для автоматической активации (опционально)

Для мгновенной активации подписки после оплаты картой:
...
Настройте webhook URL в личном кабинете YooMoney: `https://your-domain.com/webhook/yoomoney`

**Без webhook:** пользователи могут использовать команду `/check_payment` для проверки статуса платежа.
